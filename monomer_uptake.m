...
% obtain the K_matrix
K_matrix = calc_Kmat_Microbe_monomer_surface(par_mic, par_surface, nmonomers, nmicrobes, nsurfaces);

% obtain the Vmax_matrix
Vmax_matrix = calc_Vmax_microbe_monomer_surface(par_mic, par_surface, nmonomers, nmicrobes, nsurfaces);
...
function K_matrix=calc_Kmat_Microbe_monomer_surface(par_mic, par_surface, nmonomers, nmicrobes, nsurfaces)
%
% usage
...
%     K_matrix(:,jj) = par_mic(jj).Kaff_monomer';
    K_matrix(:,jj) = par_mic.Kaff_monomer(jj)';
end
%K parameter for mineral surfaces
for jj = 1 : nsurfaces
    kk = jj+nmicrobes;
%     K_matrix(:,kk) = par_surface(jj).Kaff_monomer';
    K_matrix(:,kk) = par_surface.Kaff_monomer(jj)';
end

end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function Vmax_matrix=calc_Vmax_microbe_monomer_surface(par_mic, par_surface, nmonomers, nmicrobes, nsurfaces)
%
%usage
...
%     Vmax_matrix(:,jj) = par_mic(jj).Vmax_micb';
    Vmax_matrix(:,jj) = par_mic.Vmax_micb(jj)';
end
...
%     Vmax_matrix(:,kk) = par_surface(jj).Vmax_ads_monomer';
    Vmax_matrix(:,kk) = par_surface.Vmax_ads_monomer(jj)';
end
end
