function par = set_microbe_par_default_litter_kStrat(scal, opt, par)
...
par.decay_micb0 = cpar_litter{6}(2).*scal;  % Microbial death rate (1/day)
par.decay_micb1 = 0d0;                      % Scaling parameter to account for mortality change from respiration stress
par.gmax_micb  = cpar_litter{8}(2).*scal;   % Growth rate (1/day)
par.pro_ee0    = cpar_litter{7}(2).*scal;   % Maximum enzyme production rate (1/day)
...
par.mr_micb    = cpar_litter{3}(2).*scal;   % Microbial reference maintenance rate (1/day)
par.kappa_micb = cpar_litter{4}(2).*scal;   % Metabolic turnover rate for plastic microbe (1/day)
...
par.Vmax_micb  = cpar_litter{5}(2).*scal;       % 1 x nmonomers, maximum doc uptake rate (1/day)
